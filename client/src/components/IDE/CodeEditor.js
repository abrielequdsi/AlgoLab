...
function CodeEditor({ problemNumber, isLight }) {
    ...
    const checkCodeRunOutput = (codeRunOutput, testCase) => {
        if (codeRunOutput !== "None") {
            if (codeRunOutput[0] === "[") {
                return (
                    JSON.stringify(JSON.parse(codeRunOutput)) ===
    ...
    const handleClick = () => {
        ...
        axios
            .post("http://localhost:8080/api/codeRun/python", {
                code,
                testCases,
            })
            .then((res) => {
                setCodeRunDebug(res.data.data.codeRunDebug);
                setCodeRunOutput(res.data.data.codeRunOutput);
                console.log(codeRunOutput);
                // Count Passed Test Case
                let trueCount = 0;
                res.data.data.codeRunOutput.forEach((output, i) => {
                    checkCodeRunOutput(output, testCases[i]["output"]) &&
                        trueCount++;
                });
                setCodePassCount(trueCount);
                if (trueCount !== res.data.data.codeRunOutput.length) {
                    setBoxColor("red");
                } else {
                    setBoxColor("green");
                    axios.put(
                            ...
                            <TabHelper value={tabIndex} index={0}>
                                {codeRunOutput.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        >
                                            {codePassCount} /{" "}
                                            {codeRunOutput.length} test cases
                                            passed
                                        </Typography>
